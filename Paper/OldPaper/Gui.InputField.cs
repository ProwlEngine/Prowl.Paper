...
using Prowl.PaperUI.GUI.TextEdit;
...
public partial class Paper
{
    ...
    public struct WidgetStyle
    {
        ...
        public WidgetStyle(float itemSize)
        {
            ...
            HoveredColor = new Color(204, 204, 204);
    ...
    public bool InputField(string ID, ref string value, uint maxLength, InputFieldFlags flags, Offset x, Offset y, Size width, Size? height = null, WidgetStyle? inputstyle = null, bool invisible = false)
    {
        ...
        using (g.Node(ID).Left(x).Top(y).Width(width).Height(h).Padding(5).Enter())
        {
            ...
            else
            {
                ...
                var font = style.Font != null ? style.Font : g.DefaultFont;
            ...
            if (multiline)
            {
                Vector2 textSize = (style.Font != null ? style.Font : g.DefaultFont).CalcTextSize(value, 0, g.CurrentNode.LayoutData.InnerRect.width);
    ...
    internal static bool OnProcess(WidgetStyle style, Interactable interact, ref string Text, uint MaxLength, InputFieldFlags Flags)
    {
...

        {
            ...
            {
                searches_input_ptr[1] = text_begin + MathUtilities.Min(stb.SelectStart, stb.SelectEnd);
        ...
        if (stb.CursorFollow)
        {
            ...
            if ((Flags & InputFieldFlags.NoHorizontalScroll) == 0)
            {
                ...
                if (cursor_offset.X < stb.ScrollX)
                    stb.ScrollX = (int)MathUtilities.Max(0.0f, cursor_offset.X - scroll_increment_x);
            ...
            if (is_multiline)
            {
                float scroll_y = g.CurrentNode.VScroll;
                if (cursor_offset.Y - fontsize < scroll_y)
                    scroll_y = MathUtilities.Max(0.0f, cursor_offset.Y - fontsize);
        ...
        if (stb.SelectStart != stb.SelectEnd)
        {
            int text_selected_begin = text_begin + MathUtilities.Min(stb.SelectStart, stb.SelectEnd);
            int text_selected_end = text_begin + MathUtilities.Max(stb.SelectStart, stb.SelectEnd);

            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    // FIXME: those offsets should be part of the style? they don't play so well with multi-line selection.
            float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
            Color bg_color = style.ActiveColor;
        ...
        {
            Text = stb.Text;
            if (g.IsKeyPressed(PaperKey.Return))
            {
                g.FocusID = 0;
                return true;
            }
        ...
        {
            if (!is_multiline && g.IsKeyPressed(PaperKey.Return))
                g.FocusID = 0;
    ...
    private static void HandleKeyEvent(StbTextEditState stb, uint MaxLength, InputFieldFlags Flags)
    {
        ...
        var Ctrl = g.IsKeyDown(PaperKey.LeftControl);
        var Shift = g.IsKeyDown(PaperKey.LeftShift);
        var Alt = g.IsKeyDown(PaperKey.LeftAlt);
        ...
        switch (KeyCode)
        {
            case PaperKey.Tab:
                ...
                break;
            case PaperKey.A when Ctrl && !NoSelection:
                stb.SelectStart = 0;
                stb.SelectEnd = stb.Text.Length;
                break;
            case PaperKey.Escape:
                stb.SelectStart = 0;
                stb.SelectEnd = 0;
                break;

            case PaperKey.Insert when IsEditable:
                stb_key = StbTextEdit.ControlKeys.InsertMode;
                break;
            case PaperKey.C when Ctrl && !NoSelection:
                int selectStart = Math.Min(stb.SelectStart, stb.SelectEnd);
                int selectEnd = Math.Max(stb.SelectStart, stb.SelectEnd);

                if (selectStart < selectEnd)
                {
                    g.SetClipboard(stb.Text.Substring(selectStart, selectEnd - selectStart));
                }

                break;
            case PaperKey.X when Ctrl && !NoSelection:
                ...
                break;
            case PaperKey.V when Ctrl && IsEditable:
                OnTextInput(stb, g.GetClipboard(), MaxLength, Flags);
                break;
            case PaperKey.Z when Ctrl && IsEditable:
                stb_key = StbTextEdit.ControlKeys.Undo;
                break;
            case PaperKey.Y when Ctrl && IsEditable:
            ...
            case PaperKey.Down:
                ...
                if (Shift && !NoSelection) stb_key |= StbTextEdit.ControlKeys.Shift;
                break;
            case PaperKey.Backspace when IsEditable:
                stb_key = StbTextEdit.ControlKeys.BackSpace;
                if (Shift && !NoSelection) stb_key |= StbTextEdit.ControlKeys.Shift;
                break;
            case PaperKey.Delete when IsEditable:
            ...
            case PaperKey.End:
                ...
                break;
            case PaperKey.KeypadEnter when IsEditable && Multiline:
            case PaperKey.Return when IsEditable && Multiline:
                OnTextInput(stb, "\n", MaxLength, Flags);
                break;
        }
        ...
        while (g.InputString.Count > 0)
        {
            char c = g.InputString.Dequeue();
            OnTextInput(stb, c.ToString(), MaxLength, Flags);
        }
    }
