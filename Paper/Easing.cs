    ...
    public static partial class Paper
    {
        ...
        public static class Easing
        {
            ...
            public static float Spring(float t, float dampingRatio = 0.5f, float angularFrequency = 20.0f)
            {
                ...
                dampingRatio = MathF.Max(0.0001f, dampingRatio);
                ...
                if (dampingRatio < 1.0f) // Under-damped
                {
                    ...
                    float envelope = MathF.Exp(-dampingRatio * angularFrequency * t);
                    float exponent = angularFrequency * MathF.Sqrt(1.0f - dampingRatio * dampingRatio) * t;
                    return 1.0f - envelope * MathF.Cos(exponent);
                }
                else // Critically damped (no oscillation)
                {
                    float envelope = MathF.Exp(-angularFrequency * t);
